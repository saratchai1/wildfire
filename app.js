(function () {
  ...
  function renderDetails() {
    ...
    const surface = { asphalt: 'แอสฟัลต์', paved: 'ผิวแข็ง', concrete: 'คอนกรีต', unpaved: 'ไม่ลาดยาง', unknown: 'ยังไม่สำรวจ' }[s.surface] || s.surface;
    const readings = state.view === 'demo' ? values() : null;
    ...
    $('details').innerHTML = '<div class="detail-header-block"><div class="detail-top"><span class="detail-id">' + s.id + '</span><span class="detail-status ' + (s.conditional ? 'warn' : '') + '">' + (s.coordinateSource === 'USER_DMS' ? 'พิกัดผู้ใช้ · รอสำรวจ' : s.conditional ? 'ทางดิน · ต้องตรวจสิทธิ์' : 'รอสำรวจหน้างาน') + '</span></div><h2>' + escape(s.label) + '</h2><p class="detail-subtitle">' + escape(kit.name) + ' · ฝั่ง' + ({ E: 'ตะวันออก', W: 'ตะวันตก', N: 'เหนือ' }[s.side] || 'ยังไม่ระบุ') + '</p><div class="coordinates">' + s.lat.toFixed(7) + ', ' + s.lon.toFixed(7) + '<a href="https://www.google.com/maps/search/?api=1&query=' + s.lat + '%2C' + s.lon + '" target="_blank" rel="noopener noreferrer">เปิดพิกัดใน Google Maps ↗</a></div><div class="detail-grid"><div><span>ห่างหมุดเป้าหมาย</span><b>' + number(s.distanceM / 1000, 2) + ' กม.</b></div><div><span>ระดับ DEM โดยประมาณ</span><b>' + number(s.elevationM) + ' ม.</b></div><div><span>ผิวทางใน OSM</span><b>' + surface + '</b></div><div><span>Gateway ที่เสนอ</span><b>' + (s.gateway || 'รอทดสอบสัญญาณ') + '</b></div></div></div><div class="detail-body-block"><h3>เหตุผลที่เลือกจุดนี้</h3><p class="reason">' + escape(s.reason) + '</p><h3>สิ่งที่ต้องตรวจหน้างาน</h3><p class="field-note">' + escape(s.fieldNote) + '<br>แดด / สิทธิ์ติดตั้ง / ไหล่ทาง / วิทยุ: ยังไม่ผ่านการสำรวจ</p>' + telemetry + '<a class="detail-doc" href="https://www.openstreetmap.org/way/' + (s.wayId || s.nearbyRoad?.wayId || '') + '" target="_blank" rel="noopener noreferrer">ตรวจแนวทางอ้างอิง OSM #' + (s.wayId || s.nearbyRoad?.wayId || '') + ' ↗</a></div><div class="detail-power-block"><div class="power-card"><p class="eyebrow">SOLAR + BATTERY</p><div class="power-line"><span>แผงโซลาร์</span><b>' + kit.panelWp + ' Wp</b></div><div class="power-line"><span>LiFePO₄</span><b>' + kit.batteryV + ' V · ' + kit.batteryAh + ' Ah</b></div><div class="power-line"><span>งบโหลดเฉลี่ย</span><b>' + kit.averageW + ' W</b></div><div class="autonomy-bar"><span style="width:' + Math.min(100, p.autonomyHours / 168 * 100) + '%"></span></div><div class="power-line"><span>สำรองเมื่อไม่มีแดด</span><b>' + number(p.autonomyHours, 1) + ' ชม.</b></div><small>สมมติแบตเต็ม · เผื่อความจุเสื่อม 20% แล้ว ไม่ใช่ค่าที่วัดจากอุปกรณ์จริง</small></div><a class="detail-doc" href="./docs/NORTH_STATION.md" target="_blank" rel="noopener">อ่าน Design และสูตรคำนวณ ↗</a></div>';
  }
  ...
  function fit() {
    resize();
    const points = [...window.RoadsideSiting.circle(plan.target, plan.studyRadiusM), ...stations].map(p => C.world(p.lat, p.lon));
  ...
  $('export').onclick = () => {
    ...
    const url = URL.createObjectURL(new Blob([content], { type: mime })), anchor = document.createElement('a'); anchor.href = url; anchor.download = 'wildfire-roadside-' + stations.length + '-stations.' + format; document.body.appendChild(anchor); anchor.click(); anchor.remove(); setTimeout(() => URL.revokeObjectURL(url), 1000); toast('ส่งออก ' + stations.length + ' หมุดแล้ว · สถานะจุดเสนอสำรวจ ไม่ใช่ฐานรากที่อนุมัติ');
  };
