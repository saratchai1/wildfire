...
const C=require('../core.js'),S=require('../siting.js');
const out=path.resolve('site');fs.rmSync(out,{recursive:true,force:true});fs.mkdirSync(out,{recursive:true});
const files=['index.html','v1.html','styles.css','core.js','core-v1.js','app.js','app-v1.js','history.js','history.css','siting.js','siting-ui.js','siting.css','data/context.js','data/context.json','data/plan.js','data/plan-v2.json','data/plan-v1.js','data/plan-v1-compare.js','docs/DESIGN.md','docs/DESIGN_V2.md','docs/LEGACY_REFERENCE.md'];
...
fs.writeFileSync(path.join(out,'version.json'),JSON.stringify({sourceCommit:process.env.GITHUB_SHA||'local',planVersion:plan.version,stationCount:plan.stations.length,mode:'PROPOSED_PLAN_CONDITIONAL_SCREENING_SYNTHETIC_DEMO',generatedAt:new Date().toISOString()},null,2));console.log('Built nine-roadside-station 2km application with v1 preserved.');
